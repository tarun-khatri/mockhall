...
import {
  generator,
  DS_OPTIONS,
  type DataSufficiencyFacts,
} from "../../../src/content/generators/reasoning/data-sufficiency";
import {
  answerSet,
  verify,
  verifyDs,
} from "../../../src/content/verify/reasoning/data-sufficiency";
...
function sanity(res: GenResult<DataSufficiencyFacts>): string[] {
  ...
  if (JSON.stringify(q.options) !== JSON.stringify(DS_OPTIONS))
    p.push("options not in the standard order");
  if (!f.I.length || !f.II.length) p.push("empty statement");
  if (f.I.length > 3 || f.II.length > 3) p.push("statement too long");
  if (!/\*\*I\.\*\*/.test(q.prompt) || !/\*\*II\.\*\*/.test(q.prompt))
    p.push("statements not shown");
  ...
  if (f.kind === "family" && (f.people.length < 5 || f.people.length > 6))
    p.push("family size");
  if (f.kind === "direction")
    for (const c of [...f.I, ...f.II])
      if (c.t === "exact" && (c.d < 1 || c.d > 30)) p.push(`distance ${c.d}`);
  const names =
    f.kind === "coding" ? [] : f.kind === "direction" ? f.points : f.people;
  if (names.includes("I"))
    p.push("letter I used as a name (clashes with statement I)");
  return p;
}
...
describe("data sufficiency — hand-checked cases", () => {
  ...
  it("direction: exact chain vs due-north without distance", () => {
    ...
    expect(verifyDs(f)).toBe(0);
    const g: DataSufficiencyFacts = {
      ...f,
      II: [{ t: "line", a: "R", b: "Q", dir: "E" }],
      ask: { t: "dir", a: "P", b: "Q" },
    };
    expect(verifyDs(g)).toBe(0); // II alone does not reach P
    const k: DataSufficiencyFacts = {
      ...f,
      I: [{ t: "line", a: "P", b: "R", dir: "N" }],
      II: [{ t: "line", a: "R", b: "Q", dir: "E" }],
      ask: { t: "dir", a: "P", b: "Q" },
    };
