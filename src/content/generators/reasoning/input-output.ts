...
const SUBTYPES: readonly SubtypeDef[] = [
  { id: "word-arrangement", label: "Word arrangement", weight: 1 },
  {
    ...
    label: "Number arrangement & operations",
    weight: 1,
  },
...
function pickRule(
  ...
  sub: string,
  d: Difficulty,
): { rule: IoRule; words: number; nums: number } {
  ...
  if (sub === "word-arrangement") {
    ...
    const cycle =
      d === "easy" || (d === "medium" && rng.chance(0.5))
        ? [[mv("word", order(), end())]]
        : alt("word");
  ...
  if (d === "easy" || (d === "hard" && rng.chance(0.5))) {
    ...
    const cycle = [
      [mv("word", order(), e)],
      [mv("num", order(), e === "left" ? "right" : "left")],
    ];
    return {
      rule: { cycle: rng.chance(0.5) ? cycle : [cycle[1], cycle[0]] },
      words: half,
      nums: half,
    };
  }
  ...
  const nm = mv(
    "num",
    order(),
    rng.chance(0.7) ? (e === "left" ? "right" : "left") : e,
    d === "extreme" ? randomOp(rng) : noOp,
  );
  return {
    rule: { cycle: [rng.chance(0.7) ? [w, nm] : [nm, w]] },
    words: half,
    nums: half,
  };
}
...
function moveText(m: IoMove): string {
  ...
  const where =
    m.end === "left"
      ? "goes to the left end (just after the elements already arranged there)"
      : "goes to the right end (just before the elements already arranged there)";
...
function makeInput(
  ...
  const ws = rng.sample(
    d === "easy"
      ? WORDS.filter((w, i) => WORDS.findIndex((x) => x[0] === w[0]) === i)
      : WORDS,
    words,
  );
  const pool: number[] = [];
...
function build(rng: Rng, sub: string, d: Difficulty): Built {
  for (let attempt = 0; attempt < 400; attempt++) {
    ...
    return {
      facts: { input, shown, rule, questions: [] },
      lines: sim.lines,
      last: sim.last,
    };
  }
...
function stepLines(lines: string[][], upTo: number): Rich[] {
  const out: Rich[] = [];
  for (let s = 3; s <= upTo; s++)
    out.push(`${stepName(s)}: ${fmtLine(lines[s])}`);
  return out;
}
...
function elementOptions(
  ...
  candidates: string[],
  line: readonly string[],
): { options: Rich[]; answerIndex: number } {
  const ds: string[] = [];
  for (const c of [...candidates, ...rng.shuffle([...line])])
    if (c !== correct && !ds.includes(c)) ds.push(c);
...
function makeQuestions(rng: Rng, b: Built, rule: IoRule, d: Difficulty): Q[] {
  ...
  const builders: Record<string, () => Q | null> = {
    "which-step": () => {
      // pick the answer's letter first, then a step that leaves room for that many smaller options
      const r = rng.int(0, 4);
      const ks = steps.filter((s) => s - r >= 1);
      const k = ks.length ? rng.pick(ks) : rng.pick(steps);
        ...
        draft: {
          ...
          solution: {
            steps: [
              `Rule: ${rt}`,
              ...stepLines(lines, k),
              `The given line is **${stepName(k)}**.`,
            ],
            shortcut:
              "Count how many elements already sit in their final places at the ends — each step adds one (or two).",
    ...
    "last-step": () => {
      if (last < 5) return null; // not enough room below the answer for a uniform letter
      const choices = numericChoices(rng, last, {
        rank: rng.int(0, 4),
        format: (v) => stepName(v),
        mistakes: [
          {
            value: lines[0].length,
            why: "one step per element, forgetting the last element needs no move",
          },
      ...
      return {
        ...
        draft: {
          ...
          solution: {
            steps: [
              `Rule: ${rt}`,
              ...stepLines(lines, last),
              `After ${stepName(last)} every element is arranged, so the last step is **${stepName(last)}**.`,
            ],
            shortcut:
              "The steps needed = elements that actually have to move; a final element already in place needs no step.",
    ...
    position: () => {
      const k = kStep();
      const r = rng.int(0, 4);
      const n = lines[k].length;
      const cands = lines[k]
        .flatMap((el) =>
          (["left", "right"] as const).map((from) => ({
            el,
            from,
            p: posFrom(lines[k], el, from),
          })),
        )
        .filter((c) => c.p - r >= 1 && c.p - r + 4 <= n);
      if (!cands.length) return null;
      const { el, from, p } = rng.pick(cands);
      const other = from === "left" ? "right" : "left";
      const mistakes = [
        {
          value: posFrom(lines[k], el, other),
          why: "counted from the other end",
        },
        ...(lines[k - 1].includes(el)
          ? [
              {
                value: posFrom(lines[k - 1], el, from),
                why: "used the previous step",
              },
            ]
          : []),
        ...(k < last && lines[k + 1].includes(el)
          ? [
              {
                value: posFrom(lines[k + 1], el, from),
                why: "used the next step",
              },
            ]
          : []),
      ];
      const choices = numericChoices(rng, p, {
        format: (v) => `${ordinal(v)} from the ${from} end`,
        mistakes,
        step: 1,
        min: 0,
        rank: r,
        ...
        draft: {
          ...
          solution: {
            steps: [
              `Rule: ${rt}`,
              ...stepLines(lines, k),
              `In ${stepName(k)}, '${el}' is **${ordinal(p)} from the ${from} end**.`,
            ],
    ...
    nth: () => {
      ...
      const cands = [mirror, prev, line[idx - 1], line[idx + 1]].filter(
        (x): x is string => !!x,
      );
        ...
        draft: {
          ...
          solution: {
            steps: [
              `Rule: ${rt}`,
              ...stepLines(lines, k),
              `${ordinal(pos)} from the ${from} end of ${stepName(k)}: **${correct}**.`,
            ],
            shortcut:
              "Write only the step asked, then count from the named end.",
    ...
    middle: () => {
      ...
      for (let i = 0; i < line.length; i++)
        for (let j = i + 4; j < line.length; j += 2) pairs.push([i, j]);
      ...
      const cands = [
        ...
        line[i + 1],
        line[j - 1],
      ].filter((x): x is string => !!x && x !== a && x !== b);
      const ch = elementOptions(
        rng,
        correct,
        cands.length >= 4
          ? cands
          : [...cands, ...line.filter((x) => x !== a && x !== b)],
        line.filter((x) => x !== a && x !== b),
      );
      return {
        ...
        draft: {
          ...
          solution: {
            ...
            shortcut:
              "Midway position = (position of one + position of the other) ÷ 2, counted from the same end.",
    ...
    "count-between": () => {
      ...
      const line = lines[k];
      const r = rng.int(0, 4);
      const top = Math.max(4, line.length - 2);
      const pairs: [number, number][] = [];
      for (let i = 0; i < line.length; i++)
        for (let j = i + 1; j < line.length; j++)
          if (j - i - 1 >= r && j - i - 1 - r + 4 <= top) pairs.push([i, j]);
      if (!pairs.length) return null;
      const [i, j] = rng.pick(pairs);
      const [a, b] = rng.chance(0.5) ? [line[i], line[j]] : [line[j], line[i]];
      const cnt = j - i - 1;
      ...
      const mistakes = [
        ...
        ...(prev.includes(a) && prev.includes(b)
          ? [
              {
                value: Math.abs(prev.indexOf(a) - prev.indexOf(b)) - 1,
                why: "used the previous step",
              },
            ]
          : []),
      ];
      const choices = numericChoices(rng, cnt, {
        format: (v) => String(v),
        mistakes,
        step: 1,
        allowZero: true,
        ...
        draft: {
          ...
          solution: {
            steps: [
              `Rule: ${rt}`,
              ...stepLines(lines, k),
              `Between '${line[i]}' (${ordinal(i + 1)}) and '${line[j]}' (${ordinal(j + 1)}): ${j + 1} − ${i + 1} − 1 = **${cnt}**.`,
            ],
    ...
    sum: () => {
      ...
      const prev = lines[k - 1];
      const mistakes = [
        ...(isNum(prev[i]) && isNum(prev[j])
          ? [
              {
                value: Number(prev[i]) + Number(prev[j]),
                why: "used the previous step",
              },
            ]
          : []),
        ...(k < last && isNum(lines[k + 1][i]) && isNum(lines[k + 1][j])
          ? [
              {
                value: Number(lines[k + 1][i]) + Number(lines[k + 1][j]),
                why: "used the next step",
              },
            ]
          : []),
        ...(isNum(line[line.length - 1 - i]) && isNum(line[line.length - 1 - j])
          ? [
              {
                value:
                  Number(line[line.length - 1 - i]) +
                  Number(line[line.length - 1 - j]),
                why: "counted from the right end",
              },
            ]
          : []),
      ];
      const choices = numericChoices(rng, total, {
        format: (v) => String(v),
        mistakes,
        step: rng.pick([2, 3, 5]),
      });
        ...
        draft: {
          ...
          solution: {
            steps: [
              `Rule: ${rt}`,
              ...stepLines(lines, k),
              `${ordinal(i + 1)} from the left = ${line[i]}, ${ordinal(j + 1)} from the left = ${line[j]}.`,
              `${line[i]} + ${line[j]} = **${total}**.`,
            ],
            shortcut:
              "Work out only the asked step; the arithmetic change applies once, when a number is moved.",
  ...
  const order = [
    ...must,
    ...rng.shuffle(pool.filter((x) => !must.includes(x))),
  ];
  const out: Q[] = [];
  ...
  if (out.length < 5)
    throw new Error(`${META.name}: only ${out.length} questions`);
  return rng.shuffle(out);
}

export const generator = defineGenerator<InputOutputFacts>(
  META,
  SUBTYPES,
  ({ meta, seed, difficulty, subtype, rng }) => {
    ...
    const kinds =
      subtype.id === "word-arrangement"
        ? "word"
        : subtype.id === "number-arrangement"
          ? "number"
          : "word and number";
    const stimulus = [
    ...
    const item = makeSet(meta, seed, {
      ...
      questions: qs.map((q) => ({
        ...q.draft,
        tags: [...(q.draft.tags ?? []), `io:${subtype.id}`],
      })),
    });
