...
import { DS_OPTIONS, DS_SHORT, type Category, type DsDraft } from './data-sufficiency/common';
...
import { buildDirection, type DsDirectionFacts } from './data-sufficiency/direction';
...
export type { DsRankingFacts, RankClue, RankAsk, RankAttr } from './data-sufficiency/ranking';
export type { DsFamilyFacts, FamClue, FamWord } from './data-sufficiency/family';
export type { DsDirectionFacts, DirClue, DirAsk, Dir4 } from './data-sufficiency/direction';
export type { DsSeatingFacts, SeatClue, SeatAsk, SeatLayout } from './data-sufficiency/seating';
export type { DsCodingFacts, CodedSentence, CodeAsk } from './data-sufficiency/coding';
export { DS_OPTIONS } from './data-sufficiency/common';

export type DataSufficiencyFacts = DsRankingFacts | DsFamilyFacts | DsDirectionFacts | DsSeatingFacts | DsCodingFacts;

const META: GenMeta = { name: 'reasoning.data-sufficiency', version: 1, subject: 'reasoning', chapter: 'data-sufficiency' };
...
export const generator = defineGenerator<DataSufficiencyFacts>(META, SUBTYPES, ({ meta, seed, difficulty, subtype, rng }) => {
  ...
  if (!draft) throw new Error(`${meta.name}: could not build ${subtype.id}/${difficulty} for category ${target}`);
  const { res } = draft;
  const verdict = (a: Set<string>) => (a.size === 1 ? '**sufficient**' : '**not sufficient**');
  const steps: Rich[] = [
    `Statement I alone: ${draft.say(res.ansI)} → ${verdict(res.ansI)}.`,
    `Statement II alone: ${draft.say(res.ansII)} → ${verdict(res.ansII)}.`,
  ];
  if (target >= 3) steps.push(`I and II together: ${draft.say(res.ansBoth)} → ${verdict(res.ansBoth)}.`);
