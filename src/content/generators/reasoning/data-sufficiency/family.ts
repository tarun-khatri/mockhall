...
function kinOf(w: Omit<World, "rel">, a: number, b: number): Kin {
  ...
  const sib = (x: number, y: number) =>
    x >= 0 && y >= 0 && x !== y && fa[x] >= 0 && fa[x] === fa[y];
  ...
  if (par(b).some((p) => par(p).includes(a)))
    return pk("grandfather", "grandmother");
  if (par(a).some((p) => par(p).includes(b)))
    return pk("grandson", "granddaughter");
  if (sp[b] >= 0 && par(sp[b]).includes(a))
    return pk("father-in-law", "mother-in-law");
  if (sp[a] >= 0 && par(sp[a]).includes(b))
    return pk("son-in-law", "daughter-in-law");
  if ((sp[b] >= 0 && sib(a, sp[b])) || (sp[a] >= 0 && sib(sp[a], b)))
    return pk("brother-in-law", "sister-in-law");
  if (par(b).some((p) => sib(a, p) || (sp[a] >= 0 && sib(sp[a], p))))
    return pk("uncle", "aunt");
  if (par(a).some((p) => sib(b, p) || (sp[b] >= 0 && sib(sp[b], p))))
    return pk("nephew", "niece");
    ...
    for (let code = 0; code < total; code++) {
      ...
      while (changed && !conflict) {
        ...
        for (let i = 0; i < n && !conflict; i++) {
          ...
          for (let j = 0; j < n; j++)
            if (p[j] >= 0 && M[p[j]].includes(i)) setG(j, gen[i] + 1);
        }
      }
      ...
      for (let gm = 0; gm < 1 << (couples + singles.length); gm++) {
        ...
        const fa = p.map((c) =>
          c < 0 ? -1 : g[M[c][0]] === 0 ? M[c][0] : M[c][1],
        );
        const mo = p.map((c) =>
          c < 0 ? -1 : g[M[c][0]] === 0 ? M[c][1] : M[c][0],
        );
        const base = { g, sp, fa, mo };
        const rel = new Uint8Array(n * n);
        for (let a = 0; a < n; a++)
          for (let b = 0; b < n; b++)
            if (a !== b) rel[a * n + b] = KIN.indexOf(kinOf(base, a, b));
        out.push({ ...base, rel });
      }
    }
  }
...
function wordMatches(w: FamWord, k: Kin): boolean {
  if (w in NEUTRAL)
    return (NEUTRAL[w as keyof typeof NEUTRAL] as readonly string[]).includes(
      k,
    );
  return w === k;
}
...
function neutralOf(k: Kin): FamWord | null {
  for (const [w, pair] of Object.entries(NEUTRAL))
    if ((pair as readonly string[]).includes(k)) return w as FamWord;
  return null;
}
...
export function buildFamily(
  ...
  d: Difficulty,
  target: Category,
): DsDraft<DsFamilyFacts> | null {
  const [n, couples, threeGen] = (
    {
      easy: [5, 2, true],
      medium: [6, 2, true],
      hard: [6, 2, true],
  ...
  for (let a = 0; a < n; a++)
    for (let b = 0; b < n; b++) {
      ...
      if (!direct || rng.chance(0.3))
        atoms.push({ t: "rel", a: people[a], b: people[b], w: k });
      const neu = neutralOf(k);
      if (
        neu &&
        (neu !== "sibling" && neu !== "spouse" ? true : a < b) &&
        rng.chance(0.6)
      )
        atoms.push({ t: "rel", a: people[a], b: people[b], w: neu });
    }
  for (let a = 0; a < n; a++) {
    if (rng.chance(0.5))
      atoms.push({
    ...
    if (rng.chance(0.25))
      atoms.push({ t: "married", a: people[a], yes: truth.sp[a] >= 0 });
    if (
      d !== "easy" &&
      !truth.fa.includes(a) &&
      !truth.mo.includes(a) &&
      ...
      atoms.push({ t: "childless", a: people[a] });
  }
  ...
  const say = (ans: Set<string>): string => {
    const xs = [...ans].sort();
    return xs.length === 1
      ? `${ask.a} is ${term(xs[0])} of ${ask.b}`
      : `${ask.a} could be ${listOr(xs.map(term))} of ${ask.b}`;
  };
  const both = worlds.filter((w) =>
    [...res.I, ...res.II].every((c) => holds(w, n, idx, c)),
  );
  let visual: VisualSpec | undefined;
  if (both.length) {
    const w = both.includes(truth) ? truth : both[0];
    const gen = generations(w, n);
    const members: FamilyMember[] = people.map((name, i) => ({
      id: name,
      name,
      gender: w.g[i] === 0 ? "m" : "f",
      generation: gen[i],
    ...
    for (let i = 0; i < n; i++) {
      if (w.sp[i] > i)
        links.push({ type: "spouse", a: people[i], b: people[w.sp[i]] });
      if (w.fa[i] >= 0)
        links.push(
          { type: "parent", parent: people[w.fa[i]], child: people[i] },
          { type: "parent", parent: people[w.mo[i]], child: people[i] },
        );
    }
    visual = {
      type: "family",
      ...
      caption:
        both.length === 1
          ? "The only family that fits I and II together"
          : "One family that fits I and II together",
    };
  }
  return {
    facts: {
    ...
    say,
    shortcut:
      "Draw the tree with + (male) and − (female). A statement is sufficient only if it fixes both the link between the two persons and the gender of the first one.",
    visual,
...
function generations(w: World, n: number): number[] {
  ...
  for (let it = 0; it < 3 * n; it++)
    for (let i = 0; i < n; i++) {
      ...
      if (w.fa[i] >= 0 && Number.isNaN(gen[w.fa[i]])) {
        gen[w.fa[i]] = gen[i] - 1;
        gen[w.mo[i]] = gen[i] - 1;
      }
      for (let j = 0; j < n; j++)
        if ((w.fa[j] === i || w.mo[j] === i) && Number.isNaN(gen[j]))
          gen[j] = gen[i] + 1;
    }
  const lo = Math.min(...gen);
