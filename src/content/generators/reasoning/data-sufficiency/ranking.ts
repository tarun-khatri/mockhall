...
export type RankAsk =
  | { t: "who"; k: number; from: "top" | "bottom" }
  | { t: "count"; a: string }
  | { t: "is-top"; a: string };
...
const W: Record<
  RankAttr,
  { more: string; less: string; most: string; least: string; have: string }
> = {
  height: {
    more: "taller than",
    less: "shorter than",
  ...
  weight: {
    ...
    most: "heaviest",
    least: "lightest",
    have: "has a different weight",
  },
  marks: {
    more: "scored more marks than",
    less: "scored fewer marks than",
    most: "highest scorer",
    least: "lowest scorer",
    have: "scored different marks in a test",
  },
};
...
function answerOf(order: readonly string[], ask: RankAsk): string {
  if (ask.t === "who")
    return ask.from === "top" ? order[ask.k - 1] : order[order.length - ask.k];
...
export function rankClueText(c: RankClue, attr: RankAttr, n: number): string {
  ...
  switch (c.t) {
    ...
    case "rank": {
      if (c.k === 1)
        return `${c.a} is the ${c.from === "top" ? w.most : w.least}.`;
      if (c.from === "top") {
        const k = c.k - 1;
        return attr === "marks"
      ...
      const below = c.k - 1;
      if (below * 2 === n - 1 && n % 2 === 1)
        return `${c.a} is exactly in the middle when arranged by ${attr === "marks" ? "marks" : attr}.`;
      return `${c.a} is the ${ORD_WORD[c.k]} ${w.least}.`;
    }
    case "between":
      return attr === "marks"
        ? `${c.a} scored more marks than ${c.lo} but fewer than ${c.hi}.`
        : `${c.a} is ${w.more} ${c.lo} but ${w.less} ${c.hi}.`;
    case "not":
...
function askText(ask: RankAsk, attr: RankAttr, people: string[]): string {
  ...
  if (ask.t === "count")
    return attr === "marks"
      ? `How many persons scored more marks than ${ask.a}?`
      : `How many persons are ${w.more} ${ask.a}?`;
...
export function buildRanking(
  ...
  const ask: RankAsk =
    d === "easy" || r < 0.55
      ? {
          t: "who",
          k: d === "easy" ? 1 : rng.pick([1, 1, 2, 2, 3].filter((k) => k < n)),
          from: rng.pick(["top", "bottom"] as const),
        }
      : r < 0.8
        ? { t: "count", a: rng.pick(people) }
        : {
            t: "is-top",
            a: rng.chance(0.5) ? truth[0] : rng.pick(truth.slice(1)),
          };
  ...
  for (const a of people)
    for (const b of people)
      if (
        a !== b &&
        pos(a) < pos(b) &&
        pos(b) - pos(a) <= (d === "easy" ? 2 : 3)
      )
        atoms.push({ t: "gt", a, b });
  for (let i = 0; i < n; i++) {
    if (i < 3) atoms.push({ t: "rank", a: truth[i], k: i + 1, from: "top" });
    if (n - i <= 2)
      atoms.push({ t: "rank", a: truth[i], k: n - i, from: "bottom" });
  }
  if (d !== "easy") {
    for (let i = 1; i < n - 1; i++)
      atoms.push({
        t: "between",
        a: truth[i],
        lo: truth[Math.min(n - 1, i + rng.int(1, 2))],
        hi: truth[Math.max(0, i - rng.int(1, 2))],
      });
    for (let i = 1; i < n; i++)
      if (rng.chance(0.4)) atoms.push({ t: "not", a: truth[i], end: "top" });
    for (let i = 0; i < n - 1; i++)
      if (rng.chance(0.3)) atoms.push({ t: "not", a: truth[i], end: "bottom" });
  }
  const sc = worldScenario<RankClue>(
    orders.length,
    atoms,
    (c) => JSON.stringify(c),
    (c, w) => holds(orders[w], c),
    (w) => answerOf(orders[w], ask),
  );
  const sizes = sizesFor(d, [1, 2], [1, 2], [2, 3], [2, 3]);
  const res = findPair(rng, sc, target, sizes, 45);
  if (!res) return null;
  const text = (cl: RankClue[]) =>
    rng
      .shuffle(cl)
      .map((c) => rankClueText(c, attr, n))
      .join(" ");
  const say = (ans: Set<string>): string => {
    ...
    if (ask.t === "who") {
      ...
      return xs.length === 1
        ? `${xs[0]} is ${word}`
        : `${word} could be ${listOr(xs)}`;
    }
    if (ask.t === "count") {
      const more = attr === "marks" ? "scored more marks than" : w.more;
      return xs.length === 1
        ? `exactly ${xs[0]} ${xs[0] === "1" ? "person" : "persons"} ${attr === "marks" ? "" : xs[0] === "1" ? "is " : "are "}${more} ${ask.a}`
        : `the number of persons ${more.replace(/^scored /, "scoring ")} ${ask.a} could be ${listOr(xs)}`;
    }
    return xs.length === 1
      ? `the answer is definitely “${xs[0]}”`
      : `${ask.a} may or may not be the ${w.most}`;
  };
  const unique = orders.filter((o) =>
    [...res.I, ...res.II].every((c) => holds(o, c)),
  );
  return {
    ...
    say,
    shortcut:
      "Write the persons in a column from the top and place each clue; only the asked position needs to be fixed, not the whole order.",
    visual:
      unique.length === 1
        ? {
            type: "order",
            items: unique[0],
            label: `${w.most[0].toUpperCase() + w.most.slice(1)} → ${w.least}`,
            caption: "Order fixed by I and II together",
          }
        : undefined,
