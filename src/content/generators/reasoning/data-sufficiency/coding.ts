...
export type CodeAsk =
  { t: "code"; word: string } | { t: "meaning"; code: string };
...
function worldsAnswer(
  sents: readonly CodedSentence[],
  ask: CodeAsk,
): Set<string> {
  const words = [...new Set(sents.flatMap((s) => s.words))];
  const codes = [...new Set(sents.flatMap((s) => s.codes))];
  if (ask.t === "code" ? !words.includes(ask.word) : !codes.includes(ask.code))
    return new Set(["unknown:1", "unknown:2"]);
  ...
  const rec = (i: number) => {
    if (i === words.length) {
      if (
        !sents.every((s) => s.words.every((w) => s.codes.includes(map.get(w)!)))
      )
        return;
    ...
    for (const c of codes) {
      ...
      if (!sents.every((s) => !s.words.includes(w) || s.codes.includes(c)))
        continue;
      map.set(w, c);
      used.add(c);
      rec(i + 1);
...
export function buildCoding(
  ...
  for (let t = 0; t < 60 && sentences.length < 14; t++) {
    ...
    sentences.push({
      words,
      codes: rng.shuffle(words.map((w) => code.get(w)!)),
    });
  }
  const ask: CodeAsk = rng.chance(0.7)
    ? { t: "code", word: rng.pick(vocab) }
    : { t: "meaning", code: code.get(rng.pick(vocab))! };
  ...
  const sizes = sizesFor(d, [1, 2], [1, 2], [2, 2], [2, 3]);
  const res = findPair(rng, sc, target, sizes, 40);
  if (!res) return null;
  const text = (cl: CodedSentence[]) =>
    `In the code language, ${cl.map(sentenceText).join(" and ")}.`;
  const quoted = ask.t === "code" ? `'${ask.word}'` : `'${ask.code}'`;
  const say = (ans: Set<string>): string => {
    const xs = [...ans].sort();
    if (xs[0].startsWith("unknown"))
      return `${quoted} does not appear at all, so it cannot be decoded`;
    if (ask.t === "code")
      return xs.length === 1
        ? `the code for ${quoted} is '${xs[0]}'`
        : `the code for ${quoted} could be ${listOr(xs.map((x) => `'${x}'`))}`;
    return xs.length === 1
      ? `${quoted} means '${xs[0]}'`
      : `${quoted} could mean ${listOr(xs.map((x) => `'${x}'`))}`;
  };
  return {
    ...
    question:
      ask.t === "code"
        ? `What is the code for '${ask.word}' in a certain code language?`
        : `In a certain code language, what does the code '${ask.code}' stand for?`,
    I: text(res.I),
    II: text(res.II),
    say,
    shortcut:
      "A word’s code is the code common to every sentence containing that word — after removing codes already fixed for other words. If two codes are still possible, the data are not sufficient.",
