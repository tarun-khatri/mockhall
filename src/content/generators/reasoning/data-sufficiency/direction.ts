...
import { CAP_NUM_WORD, findPair, listAnd, listOr, sizesFor, type Category, type DsDraft, type Scenario } from './common';

export type Dir4 = 'N' | 'S' | 'E' | 'W';
export type DirClue = { t: 'exact'; a: string; b: string; d: number; dir: Dir4 } | { t: 'line'; a: string; b: string; dir: Dir4 };
...
const VEC: Record<Dir4, [number, number]> = { N: [0, 1], S: [0, -1], E: [1, 0], W: [-1, 0] };
const NAME: Record<Dir4, string> = { N: 'north', S: 'south', E: 'east', W: 'west' };
...
export function dirClueText(c: DirClue): string {
  return c.t === 'exact' ? `${c.a} is ${c.d} m to the ${NAME[c.dir]} of ${c.b}.` : `${c.a} is due ${NAME[c.dir]} of ${c.b}.`;
}

/** All (dx, dy) of a relative to b over consistent placements, or null if a and b are not linked. */
function displacements(cl: readonly DirClue[], a: string, b: string): [number, number][] | null {
  const Sx = cl.reduce((s, c) => s + (c.t === 'exact' ? Math.abs(VEC[c.dir][0]) * c.d : c.dir === 'E' || c.dir === 'W' ? 1 : 0), 1);
  const Sy = cl.reduce((s, c) => s + (c.t === 'exact' ? Math.abs(VEC[c.dir][1]) * c.d : c.dir === 'N' || c.dir === 'S' ? 1 : 0), 1);
  const Bx = 2 * Sx + 2;
  const By = 2 * Sy + 2;
  ...
  while (grew) {
    grew = false;
    for (const c of cl) {
      if (reached.has(c.a) && !reached.has(c.b)) (order.push({ c, from: c.a, to: c.b }), reached.add(c.b), (grew = true));
      else if (reached.has(c.b) && !reached.has(c.a)) (order.push({ c, from: c.b, to: c.a }), reached.add(c.a), (grew = true));
    }
  }
  ...
  const rec = (i: number) => {
    ...
    const [ux, uy] = VEC[c.dir];
    const lens = c.t === 'exact' ? [c.d] : Array.from({ length: ux !== 0 ? Bx : By }, (_, k) => k + 1);
    for (const L of lens) {
...
export function buildDirection(rng: Rng, d: Difficulty, target: Category): DsDraft<DsDirectionFacts> | null {
  const n = { easy: 4, medium: 5, hard: 5, extreme: 6 }[d];
  const pool = rng.pick(['PQRSTUV', 'ABCDEFG', 'JKLMNPQ', 'EFGHJKL', 'KLMNPQR']);
  ...
  for (const a of points) for (const b of points) if (a !== b) pairs.push([a, b]);
  const diag = pairs.filter(([a, b]) => C(a)[0] !== C(b)[0] && C(a)[1] !== C(b)[1]);
  const [qa, qb] = diag.length && rng.chance(0.75) ? rng.pick(diag) : rng.pick(pairs);
  const ask: DirAsk = { t: d !== 'easy' && rng.chance(0.3) ? 'dist' : 'dir', a: qa, b: qb };
  const atoms: DirClue[] = [];
  ...
  const answersUncached = (cl: readonly DirClue[]): Set<string> => {
    ...
    for (const [x, y] of disp) out.add(x === 0 && y === 0 ? 'same' : ask.t === 'dir' ? dirLabel(x, y) : `d2:${x * x + y * y}`);
    return out;
  };
  ...
  const say = (ans: Set<string>): string => {
    const xs = [...ans].sort();
    if (xs[0].startsWith('unlinked')) return `${qa} cannot be linked to ${qb}, so its position relative to ${qb} is unknown`;
    if (ask.t === 'dir') return xs.length === 1 ? `${qa} is to the ${xs[0]} of ${qb}` : `${qa} could be to the ${listOr(xs)} of ${qb}`;
    return xs.length === 1 ? `${qa} is ${dist(xs[0])} from ${qb}` : `the distance could be ${listOr(xs.slice(0, 3).map(dist))}${xs.length > 3 ? ' and more' : ''}`;
  };
  return {
    facts: { kind: 'direction', points, I: res.I, II: res.II, ask },
    context: `${CAP_NUM_WORD[n]} points — ${listAnd(points)} — lie on a flat field.`,
    question: ask.t === 'dir' ? `In which direction is point ${qa} with respect to point ${qb}?` : `What is the shortest distance between points ${qa} and ${qb}?`,
    I: text(res.I),
    II: text(res.II),
    say,
    shortcut:
    ...
    tags: ['ds:direction', `direction:${ask.t === 'dir' ? 'relative-direction' : 'shortest-distance'}`],
    res,
  };
}
