...
export type SeatAsk =
  | { t: "rel"; of: string; k: number; side: "left" | "right" }
  | { t: "end"; side: "left" | "right" }
  | { t: "middle" }
  | { t: "opp"; of: string };
...
function relWord(k: number, side: "left" | "right"): string {
  return k === 1
    ? `to the immediate ${side} of`
    : `${ORD_WORD[k]} to the ${side} of`;
}
...
export function seatClueText(c: SeatClue, layout: SeatLayout): string {
  switch (c.t) {
    ...
    case "gap":
      return c.k === 0
        ? `${c.a} and ${c.b} sit together.`
        : `Only ${NUM_WORD[c.k]} ${c.k === 1 ? "person sits" : "persons sit"} between ${c.a} and ${c.b}.`;
    case "end":
...
export function buildSeating(
  ...
  const worlds: string[][] = [];
  if (layout === "linear")
    for (const p of perms(n)) worlds.push(p.map((i) => people[i]));
  else {
    const rot = truth.indexOf(people[0]);
    const t2 = [...truth.slice(rot), ...truth.slice(0, rot)];
    truth.splice(0, n, ...t2);
    for (const p of perms(n - 1))
      worlds.push([people[0], ...p.map((i) => people[i + 1])]);
  }
  const asks: SeatAsk[] = [];
  for (const of of people)
    for (const side of ["left", "right"] as const)
      for (let k = 1; k <= (d === "easy" ? 1 : 2); k++) {
  ...
  for (const a of people)
    for (const b of people) {
      ...
      for (const side of ["left", "right"] as const)
        for (let k = 1; k <= 3; k++)
          if (
            step(layout, n, p(b), k, side) === p(a) &&
            (k === 1 || rng.chance(0.6))
          )
            atoms.push({ t: "rel", a, b, k, side });
      if (a < b) {
        const c1: SeatClue = { t: "adj", a, b };
        if (holds(layout, truth, c1)) atoms.push(c1);
        else if (d !== "easy" && rng.chance(0.35))
          atoms.push({ t: "nadj", a, b });
        if (layout === "linear") {
          const k = Math.abs(p(a) - p(b)) - 1;
          if (k >= 1 && k <= 3) atoms.push({ t: "gap", a, b, k });
        } else if (n % 2 === 0 && holds(layout, truth, { t: "opp", a, b }))
          atoms.push({ t: "opp", a, b });
      }
    }
  if (layout === "linear")
    for (const a of people) {
  ...
  if (!res) return null;
  const text = (cl: SeatClue[]) =>
    rng
      .shuffle(cl)
      .map((c) => seatClueText(c, layout))
      .join(" ");
  const say = (ans: Set<string>): string => {
    ...
    return xs.length === 1
      ? `only ${xs[0]} ${what}`
      : `${listOr(xs)} could be the one who ${what}`;
  };
  const fixed = worlds.filter((s) =>
    [...res.I, ...res.II].every((c) => holds(layout, s, c)),
  );
    ...
    visual =
      layout === "linear"
        ? {
            ...
            rows: [
              {
                seats: fixed[0].map((name) => ({
                  name,
                  facing: "north" as const,
                })),
              },
            ],
            caption: "Arrangement fixed by I and II together",
          }
        : {
            type: "circular",
            seats: fixed[0].map((name) => ({
              name,
              facing: "inside" as const,
            })),
            caption: "Arrangement fixed by I and II together",
          };
  }
