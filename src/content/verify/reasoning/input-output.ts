...
import type { InputOutputFacts, IoMove, IoOp } from '../../generators/reasoning/input-output';
...
function run(input: readonly string[], cycle: IoMove[][], limit = 40): string[][] {
  ...
  for (let s = 0; s < limit && L + R < line.length; s++) {
    ...
    for (const m of moves) {
      ...
      for (let i = L; i < line.length - R; i++) {
        ...
        if (best === null) best = i;
          ...
          const cmp = m.kind === 'num' ? parseInt(line[i], 10) - parseInt(line[best], 10) : line[i].localeCompare(line[best], 'en');
          if ((m.order === 'asc' && cmp < 0) || (m.order === 'desc' && cmp > 0)) best = i;
        }
      }
    ...
    if (L + R >= line.length && line.join('\u0001') === prev.join('\u0001')) break; // a final step that changes nothing
    out.push(line.slice());
  }
  return out;
}
...
export function inferRun(f: InputOutputFacts): string[][] {
  ...
  const consider = (cycle: IoMove[][]) => {
    ...
    if (r.length <= f.shown.length || !f.shown.every((l, i) => same(r[i + 1], l))) return;
    if (!found) found = r;
    else if (found.length !== r.length || found.some((l, i) => !same(l, r[i]))) throw new Error('ambiguous: two rules fit the shown steps but differ later');
  };
