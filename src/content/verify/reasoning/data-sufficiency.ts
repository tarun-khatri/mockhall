...
function rankHolds(
  ...
  const need =
    c.t === "between" ? [c.a, c.lo, c.hi] : c.t === "gt" ? [c.a, c.b] : [c.a];
  ...
  const rec = (i: number) => {
    ...
    if (i === n) {
      ...
      if (ask.t === "rel") s = move(seat.get(ask.of)!, ask.k, ask.side);
        ...
        s = ask.side === "left" ? 0 : n - 1; // facing north: the left end is the west end
...
function families(n: number, couples: number, threeGen: boolean): Fam[] {
  ...
  for (let gmask = 0; gmask < 1 << n; gmask++) {
    ...
    for (const cs of coupleSets) {
      ...
      const rec = (i: number) => {
        if (i === n) {
          if (valid(n, spouse, father, mother, threeGen))
            out.push({
              male,
              spouse: spouse.slice(),
              father: father.slice(),
...
function valid(
  ...
  while (queue.length) {
    ...
    if (
      father[x] !== null &&
      (!put(father[x]!, v - 1) || !put(mother[x]!, v - 1))
    )
      return false;
    for (let y = 0; y < n; y++)
      if ((father[y] === x || mother[y] === x) && !put(y, v + 1)) return false;
  }
  if (level.some((v) => v === null)) return false; // not connected
  const span =
    Math.max(...(level as number[])) - Math.min(...(level as number[]));
  ...
  for (let x = 0; x < n; x++) {
    ...
    for (const k of ancestorsWithDepth(father, mother, s).keys())
      if (ax.has(k)) return false;
  }
...
function kinship(f: Fam, a: number, b: number): string {
  ...
  const blood = (x: number, y: number): [number, number] | null => {
    ...
    for (const [k, dx] of ax) {
      const dy = ay.get(k);
      if (dy !== undefined && (!best || dx + dy < best[0] + best[1]))
        best = [dx, dy];
    }
    return best;
  };
  const is = (d: [number, number] | null, x: number, y: number) =>
    !!d && d[0] === x && d[1] === y;
  const bl = blood(a, b);
  if (bl) {
...
function famHolds(fam: Fam, ix: (x: string) => number, c: FamClue): boolean {
  ...
  switch (c.t) {
    ...
    case "childless":
      return (
        fam.father.every((p) => p !== a) && fam.mother.every((p) => p !== a)
      );
  }
}
...
function familyAnswers(f: DsFamilyFacts, clues: FamClue[]): Set<string> {
  ...
  for (const fam of families(f.people.length, f.couples, f.threeGen))
    if (clues.every((c) => famHolds(fam, ix, c)))
      out.add(kinOf(fam, ix(f.ask.a), ix(f.ask.b)));
  return out;
}
  ...
  const S =
    ...
    clues.reduce(
      (s, c) =>
        s +
        (c.t === "exact"
          ? Math.abs(UNIT[c.dir][axis]) * c.d
          : Math.abs(UNIT[c.dir][axis])),
      0,
    );
  const R = 2 * S + 2;
...
function directionAnswers(f: DsDirectionFacts, clues: DirClue[]): Set<string> {
  ...
  for (const x of xs)
    for (const y of ys) {
      if (f.ask.t === "dist")
        out.add(x === 0 && y === 0 ? "same" : `d2:${x * x + y * y}`);
      else {
...
function codingAnswers(f: DsCodingFacts, sents: CodedSentence[]): Set<string> {
  const wordSig = (w: string) =>
    sents.map((s) => (s.words.includes(w) ? "1" : "0")).join("");
  const codeSig = (c: string) =>
    sents.map((s) => (s.codes.includes(c) ? "1" : "0")).join("");
...
export function verifyDs(f: DataSufficiencyFacts): number {
  ...
  if (!I.size || !II.size || !both.size)
    throw new Error("statements are inconsistent");
